...
class inProceedingsHandler(xml.sax.handler.ContentHandler):
    ...
    def __init__(self,authors_dict):
        ...
        self.outfile = open('edgelist95.txt','w')
            ...
            if length > 1:
                for i in range(0,length):
				    for j in range(i+1,length):
						self.outfile.write(str(self.author_list[i])+ ' ' + str(self.author_list[j]) + '\n')
            self.author_list = []
        elif name == 'author':
...
if __name__ == '__main__':
    ...
    inProceedings = inProceedingsHandler(authors_dict)
    source = open('inProceedings95.xml','r')
