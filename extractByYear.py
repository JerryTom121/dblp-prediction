...
class dblpHandler(xml.sax.handler.ContentHandler):
    def __init__(self,year1,year2):
        ...
        self.outfile = codecs.open('inproceedings9194.xml','w','utf-8')
    ...
    def endElement(self,name):
        if name == 'inproceedings':
            #self.outfile.write('</inproceedings>\n\n')
            self.buffer += '</inproceedings>\n\n'
            if self.currentYear <= self.year2 and self.currentYear >= self.year1: 
...
if __name__ == '__main__':
    dblp = dblpHandler(1991,1994)
    source = open('totalinproceedings.xml','r')
